...
func newTestServer(t *testing.T, opts ...func(*cache.Config)) (http.Handler, string) {
	t.Helper()
	...
	return cache.New(cfg).Handler(), spoolDir
}
...
func TestNarInfoSigned(t *testing.T) {
	...
	h, _ := newTestServer(t, func(cfg *cache.Config) {
		cfg.Signer = sk
	})
...
// prints and what cmd/tsnixcache's GC threshold measures). The Overview
...
func TestStoreDiskGaugesExcludeTheRootReserve(t *testing.T) {
	...
	h, _ := newTestServer(t, func(c *cache.Config) {
		c.StoreRoot = dir
		c.StoreDir = "/"
	})
...
func newZstdTestServer(t *testing.T) (*cache.Server, http.Handler, string) {
	t.Helper()
...
func newZstdTestServerDirs(t *testing.T) (*cache.Server, http.Handler, string, string) {
	t.Helper()
...
func TestZstdCacheFilesLandInSpoolSubdir(t *testing.T) {
	...
	cacheDir := filepath.Join(spoolDir, cache.ZstdCacheSubdir)
...
// barrierStore releases every concurrent PathInfo caller at the same instant, so
// requests provably overlap instead of relying on scheduling luck.
...
func (b *barrierStore) PathInfo(ctx context.Context, hashPart string) (*store.PathInfo, error) {
	b.wg.Done()
	b.wg.Wait()

	return b.fakeStore.PathInfo(ctx, hashPart)
}

// TestZstdCompressionIsSingleFlighted confirms concurrent narinfo requests for
// one path share a single compression. Without single-flight each request
// compresses the whole NAR and every loser's temp file is orphaned for the life
// of the process.
func TestZstdCompressionIsSingleFlighted(t *testing.T) {
	...
	// Incompressible payload: compression must take long enough that the racing
	// requests are genuinely inside it together.
	...
	err = os.WriteFile(filepath.Join(storePath, "data"), payload, 0o600)
	if err != nil {
		t.Fatal(err)
	}

	bs := &barrierStore{fakeStore: &fakeStore{paths: map[string]*store.PathInfo{
		testHash: {
			StorePath:  storePath,
			NarHash:    testNarHash,
			NarSize:    42,
	...
	for i := range concurrency {
		wg.Go(func() {
			codes[i] = getBytes(t, h, "/"+testHash+".narinfo").Code
		})
	}
	...
	entries, err := os.ReadDir(filepath.Join(spoolDir, cache.ZstdCacheSubdir))
	if err != nil {
		t.Fatal(err)
	}

	if len(entries) != 1 {
		t.Errorf("%d concurrent narinfo requests produced %d cache files, want 1", concurrency, len(entries))
	}
}
...
func TestImportRunsDetachedFromClientCancellation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fakeFn := cache.ImporterFunc(func(importCtx context.Context, _ *narinfo.NarInfo) error {
				if !tt.cancelBefore {
					cancel()
				}
...
func TestErrorLogsNameThePath(t *testing.T) {
	t.Run("import failure", func(t *testing.T) {
		...
		failing := cache.ImporterFunc(func(_ context.Context, _ *narinfo.NarInfo) error {
...
func narSize(t *testing.T, path string) int64 {
	t.Helper()
...
func TestConcurrentPutSameNarDoesNotInterleave(t *testing.T) {
	...
	for _, ent := range ents {
		if !ent.IsDir() && ent.Name() != testHash+".nar" {
...
func TestPutNarOverDirectoryLeavesItIntact(t *testing.T) {
	...
	dir := filepath.Join(spoolDir, cache.ZstdCacheSubdir)

	req := httptest.NewRequestWithContext(context.Background(), http.MethodPut,
		"/nar/"+cache.ZstdCacheSubdir, strings.NewReader("nar bytes"))
	w := httptest.NewRecorder()
	h.ServeHTTP(w, req)
...
func TestImportIsTimed(t *testing.T) {
	h := cache.New(cache.Config{
		...
		ServeCompression: compressNone,
		StoreDir:         nixStoreDir,
		ImportFn: cache.ImporterFunc(func(_ context.Context, _ *narinfo.NarInfo) error {
			return nil
		}),
	}).Handler()
...
func TestNarOverRealConnection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ts := httptest.NewServer(cache.New(cache.Config{
				Store:            s,
				Priority:         30,
				SpoolDir:         t.TempDir(),
