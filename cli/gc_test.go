...
func TestPruneRequiresDurableRemoval(t *testing.T) {
	storeDir := t.TempDir()
	roots := t.TempDir()
	root := filepath.Join(roots, nixHash)
	symlink(t, filepath.Join(storeDir, "aaa-pkg"), root, false)
	...
	t.Cleanup(gcroot.SetSyncDirForTesting(func(path string) error {
		...
		_, err := os.Lstat(root)
		if path == roots && errors.Is(err, os.ErrNotExist) {
			return injected
		}
	...
	result, err := pruneOldGCRoots(roots, storeDir, ageAll, true)
	require.NoError(t, err)
	require.Equal(t, 1, result.pruned)
	require.Zero(t, syncs)

	result, err = pruneOldGCRoots(roots, storeDir, ageAll, false)
	require.ErrorIs(t, err, injected)
	require.Zero(t, result.pruned)
...
func TestGCStopsAfterPruneSyncFailure(t *testing.T) {
	storeDir := t.TempDir()
	roots := t.TempDir()
	root := filepath.Join(roots, nixHash)
	symlink(t, filepath.Join(storeDir, "aaa-pkg"), root, false)
	invoked := filepath.Join(t.TempDir(), "invoked")
	t.Setenv("TSNIXCACHE_TEST_COLLECTED", invoked)
	bin := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(bin, "nix-collect-garbage"), // #nosec G306 -- executable test fixture.
		[]byte("#!/bin/sh\ntouch \"$TSNIXCACHE_TEST_COLLECTED\"\n"), 0o700))
	...
	g := &gcRunner{storeDir: storeDir, gcrootDir: roots, rules: []GCRule{{alwaysTriggerPct, ageAll}}}
	_, err := g.apply(t.Context())
	require.ErrorIs(t, err, injected)
...
func TestRunGCWatcherChecksBeforeTicking(t *testing.T) {
	...
	startTestTask(t, func() { runGCWatcher(ctx, dir, t.TempDir(), []GCRule{{alwaysTriggerPct, dur20d}}, 24*time.Hour, nil) })
	t.Cleanup(cancel)

	require.EventuallyWithT(t, func(c *assert.CollectT) {
		_, err := os.Stat(ranFile)
...
type refreshRootHandler struct{ refresh func() }

func (h refreshRootHandler) Enabled(context.Context, slog.Level) bool { return true }

func (h refreshRootHandler) Handle(_ context.Context, r slog.Record) error {
	if strings.HasPrefix(r.Message, "gc: kept entries") {
		h.refresh()
	}
...
func (h refreshRootHandler) WithAttrs([]slog.Attr) slog.Handler { return h }

func (h refreshRootHandler) WithGroup(string) slog.Handler { return h }

func TestPrunePreservesRootRefreshedAfterSelection(t *testing.T) {
	roots, store := t.TempDir(), t.TempDir()
	root := filepath.Join(roots, nixHash)
	target := filepath.Join(store, nixHash+"-path")
	require.NoError(t, os.Symlink(target, root))

	old := unix.NsecToTimespec(time.Now().Add(-48 * time.Hour).UnixNano())
	require.NoError(t, unix.UtimesNanoAt(unix.AT_FDCWD, root, []unix.Timespec{old, old}, unix.AT_SYMLINK_NOFOLLOW))
	require.NoError(t, os.WriteFile(filepath.Join(roots, "foreign"), nil, 0o600))

	refreshed := false
	previous := slog.Default()

	t.Cleanup(func() { slog.SetDefault(previous) })
	slog.SetDefault(slog.New(refreshRootHandler{refresh: func() {
		require.NoError(t, os.Symlink(target, root+".new"))
		require.NoError(t, os.Rename(root+".new", root))

		refreshed = true
	}}))

	result, err := pruneOldGCRoots(roots, store, 24*time.Hour, false)
	require.NoError(t, err)
...
func TestPruneSkipsOwnedRootsAndKeepsDryRunReadOnly(t *testing.T) {
	roots, storeDir := t.TempDir(), t.TempDir()
	root := filepath.Join(roots, nixHash)
	require.NoError(t, os.Symlink(filepath.Join(storeDir, "path"), root))
	result, err := pruneOldGCRoots(roots, storeDir, ageAll, true)
	require.NoError(t, err)
	require.Equal(t, 1, result.pruned)

	_, err = os.Stat(filepath.Join(roots, gcroot.LockDir))
	require.ErrorIs(t, err, os.ErrNotExist)
	lock, err := gcroot.Acquire(t.Context(), roots, nixHash)
	require.NoError(t, err)
