...
var (
	...
	errPushPathRequired = errors.New("push: at least one store path is required (\"-- -\" reads them from stdin)")
...
func newPushCmd() *ff.Command {
	fs := ff.NewFlagSet("push")
	to := fs.StringLong("to", "", "target cache URL (required)")
	attempts := fs.IntLong("attempts", 5, "max attempts per path before giving up (at least 1)")
	jobs := fs.IntLong("jobs", 8, "max concurrent path uploads (at least 1)")
	...
	noProgress := fs.BoolLongDefault("no-progress", false, "disable the interactive progress bars even on a terminal")
...
// argument into the lines on stdin so a closure can be piped in. ff drops a
// bare "-" ahead of every other operand, so it follows "--":
//
...
func durationFlag(fs *ff.FlagSet, name string, def time.Duration, usage string) *time.Duration {
	d := def
	fs.Value(0, name, dayDuration{&d}, usage)
