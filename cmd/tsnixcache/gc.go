...
// minSafeOlderThan is the shortest gcroot age that still protects a push in
// flight. The gcroot is written before the import starts precisely so a
// concurrent collection cannot reap a path being written, but that only holds
// while the age outlives the import.
...
// gcRootEpoch is the earliest mtime a gcroot tsnixcache wrote can plausibly
// have — the project did not exist before it. A machine with no
// battery-backed clock boots near the Unix epoch and stamps the roots it
// writes then with a 1970 mtime; once NTP steps the clock forward every one of
// them looks decades old at once, and a single tick would prune the whole
// cache and collect it. A variable so tests can move it.
...
func warnGCRuleShape(existing []GCRule, rule GCRule) {
	if rule.OlderThan < minSafeOlderThan {
		slog.Warn("gc: rule keeps gcroots for less than a minute, so a push still importing can lose its root",
...
func (c *gcRootsCollector) Collect(ch chan<- prometheus.Metric) {
	...
	ch <- prometheus.MustNewConstMetric(c.desc, prometheus.GaugeValue, float64(len(entries)))
}
...
func pruneOldGCRoots(gcrootDir, storeDir string, minAge time.Duration, dryRun bool) (pruneResult, error) {
	...
	stale := selectStaleGCRoots(gcrootDir, storeDir, time.Now().Add(-minAge), entries)
...
// diskUsedPct reports how full the filesystem holding storeDir is, as
// (Blocks-Bfree)/Blocks: bytes actually occupied over the whole device, which is
// exactly tsnixcache_store_disk_used_bytes over _total_bytes, and what the
// Overview disk tile plots. The same /metrics page shows all three, so they have
// to be reconcilable.
//
// The root reserve therefore counts as free, not as used. Measuring against
// Bavail instead counts it as used and reads ~5 points higher (57% where this
// says 52% on a 4% ext4 reserve), so a threshold of 80 fired at what the gauges
// beside it called 76%. Neither number is df's Use%, which is
// used/(used+available) and lands between the two.
func diskUsedPct(storeDir string) (int, error) {
	...
	// Pseudo-filesystems report no blocks at all. Dividing by that yields NaN
	// or +Inf, and Go leaves the conversion of either to int unspecified:
	// measured as minint on amd64 and as a saturating 100 on arm64, so the
	// same store would never be collected on one and always on the other.
	if st.Blocks == 0 {
...
func newGCCmd() *ffcli.Command {
	fs := flag.NewFlagSet("tsnixcache gc", flag.ExitOnError)

	var rules gcRulesFlag

	fs.Var(&rules, "gc-rule", `GC rule as "threshold:duration" e.g. "80:20d"; repeatable`)

	storeDir := fs.String("store-dir", defaultStoreDir,
	...
	return &ffcli.Command{
		Name: "gc",
		ShortUsage: "tsnixcache gc --gc-rule threshold:duration [--gc-rule ...] " +
