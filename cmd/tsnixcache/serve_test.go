...
func newStoreFlagSet() *flag.FlagSet {
	fs := flag.NewFlagSet("serve", flag.ContinueOnError)
	fs.String("db", "/nix/var/nix/db/db.sqlite", "")
	fs.String("store-dir", "/nix/store", "")
	fs.String("nix-store-uri", "auto", "")
	fs.String("gcroot-dir", "/nix/var/nix/gcroots/tsnixcache", "")
	fs.String("store", "", "")
...
func TestServeHTTPFinishesInFlightRequest(t *testing.T) {
	...
	serveHTTP(egCtx, eg, ln, http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		// Stands in for nix-store --import: still running when SIGTERM
...
// waitUntil polls cond until it holds. Polling a condition beats sleeping a
// guessed interval: it fails loudly rather than flakily, and costs only as long
// as the machine actually needs.
...
func TestParseTsnetSpec(t *testing.T) {
	tests := []struct {
		...
		{"trailing comma tolerated", "hostname=cache,", tsnetSpec{hostname: "cache"}, nil},
