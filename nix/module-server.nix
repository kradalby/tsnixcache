...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules =
      map (d: "d ${d}  0750 tsnixcache tsnixcache -") writableDirs
      ++ map (ts: "d ${ts.dir}  0750 tsnixcache tsnixcache -") cfg.tsnet;
    ...
    systemd.services.tsnixcache = {
      ...
      serviceConfig = {
        User = "tsnixcache";
        Group = "tsnixcache";
        # One list element per argument, shell-quoted: joined by hand, an empty
        # or space-carrying value (listen = [ "" ] asks the CLI for no local
        # listener) would collapse into the next flag, and Go's flag package
        ...
        LoadCredential =
          lib.optional (cfg.signKeyFile != null) "${signKeyCred}:${cfg.signKeyFile}"
          ++ lib.imap0 (i: ts: "${tsnetCred i}:${ts.authKeyFile}") cfg.tsnet;
