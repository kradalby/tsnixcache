...
var (
	...
	// ErrNonCanonical is returned when the leading character carries bits that
	// fall outside the decoded output. Accepting those would give a hash several
	// distinct encodings, so they must be zero.
...
//
// The encoded length need not be a whole number of bytes: 52 characters carry
// 260 bits but decode to 32 bytes. Any bit beyond the decoded output must be
// zero, otherwise the same hash would have several valid encodings and callers
// keying on the string could be fooled by a re-encoded duplicate.
