...
// FuzzDecodeString preserves bytes and the decoder's leading-zero aliases.
func FuzzDecodeString(f *testing.F) {
	for _, seed := range []string{"", "0", "z", "7z", "07z", "zzz", "000000000", "00qn3vc7r4m32c072kjnrbxd86w9slzj", "!@#", "eotu", "ABC", "\x00", "\xff"} {
		f.Add(seed)
	}

	for _, size := range []int{1, 2, 3, 4, 5, 6, 7, 8, 9, 26, 32, 52, 103} {
		f.Add(strings.Repeat("0", size))
		f.Add(strings.Repeat("z", size))
	}
	...
	f.Fuzz(func(t *testing.T, s string) {
		...
		require.Len(t, got, len(s)*5/8)
		canonical := EncodeToString(got)
		require.Equal(t, s, strings.Repeat("0", len(s)-len(canonical))+canonical)

		if ValidHashPart(s) {
			require.Len(t, got, 20)
			require.Equal(t, s, canonical)
		}
	})
}
