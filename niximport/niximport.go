...
var (
	...
	errNarTooLarge      = errors.New("niximport: decoded NAR exceeds size limit")
)

// DefaultMaxNarSize bounds decoded bytes independently of the pushed metadata.
const DefaultMaxNarSize uint64 = 16 << 30
...
type Importer struct {
	...
	MaxNarSize  uint64 // maximum decoded bytes; zero uses DefaultMaxNarSize
}

// Import verifies the spool, then streams the same descriptor into Nix.
// Decoding twice keeps unverified bytes out of the store without buffering a NAR.
func (imp *Importer) Import(ctx context.Context, ni *narinfo.NarInfo) error {
	...
	lock, err := imp.lockGCRoot(ctx, ni.StorePath)
	if err != nil {
		dropSpool(spoolPath, verified)
	...
	// the Nix import returning and the root appearing, a concurrent
	...
	createdRoot, err := imp.addGCRoot(ni.StorePath)
	if err != nil {
		dropSpool(spoolPath, verified)
	...
	if err != nil {
		if createdRoot != "" {
			// The root was absent when this import acquired exclusive ownership.
			removeErr := os.Remove(createdRoot)
			if removeErr == nil {
				removeErr = gcroot.SyncDir(imp.GCRootDir)
			}
	...
	// Retention starts when import completes, while pruning is still excluded.
	_, err = imp.addGCRoot(ni.StorePath)
	if err != nil {
		dropSpool(spoolPath, verified)
...
func (imp *Importer) lockGCRoot(ctx context.Context, storePath string) (*os.File, error) {
	if imp.GCRootDir == "" {
		return nil, nil //nolint:nilnil // Roots are explicitly disabled.
	}

	key := imp.storePathHash(storePath)
	if key == "" {
		return nil, fmt.Errorf("%w: %q", errInvalidStorePath, storePath)
	}

	return gcroot.Acquire(ctx, imp.GCRootDir, key)
}
...
func (imp *Importer) addGCRoot(storePath string) (string, error) {
	...
	rootPath := filepath.Join(imp.GCRootDir, hashPart)
	...
	if err != nil && !errors.Is(err, os.ErrNotExist) {
		return "", fmt.Errorf("niximport: inspect gcroot: %w", err)
	}
	...
	tmp := fmt.Sprintf("%s.tmp.%d.%d", rootPath, os.Getpid(), gcRootSeq.Add(1))

	err = os.Symlink(storePath, tmp)
	if err == nil {
		err = os.Rename(tmp, rootPath)
	}

	if err != nil {
		_ = os.Remove(tmp)

		return "", fmt.Errorf("niximport: gcroot symlink %s: %w", rootPath, err)
	}

	// Acknowledging an import requires a durable root, including its refresh.
	err = gcroot.SyncDir(imp.GCRootDir)
	if err != nil {
		return "", err
	}
...
func (imp *Importer) verify(ctx context.Context, f *os.File, ni *narinfo.NarInfo) (os.FileInfo, error) {
	...
	limit := imp.MaxNarSize
	if limit == 0 {
		limit = DefaultMaxNarSize
	}

	if ni.NarSize > limit || ni.NarSize >= math.MaxInt64 {
		return fi, fmt.Errorf("%w: got %d, maximum %d", errNarTooLarge, ni.NarSize, limit)
	}
	...
	// silent truncation.
	// #nosec G115 -- size is checked against MaxInt64 above.
...
// Reuse the verified descriptor; a concurrent PUT may have replaced its pathname.
func (imp *Importer) importNar(ctx context.Context, f *os.File, ni *narinfo.NarInfo) error {
	...
	return imp.importStream(ctx, dec, ni)
}
