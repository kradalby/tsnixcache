...
// Make missing real-import coverage visible even without verbose test output.
func requireNix(tb testing.TB) {
	tb.Helper()
	...
	if err != nil {
		...
		tb.Skip("nix-store not in PATH: the real import is not being exercised")
	}
}
...
// before removal, because nix-store --serve writes read-only paths.
func nixStoreTempDir(tb testing.TB) string {
	tb.Helper()

	// Nix rejects symlinks in store ancestors, including Darwin's /tmp.
...
func buildNarFor(tb testing.TB, path string) []byte {
	tb.Helper()
	...
	if err != nil {
		tb.Fatalf("nar.Write(%s): %v", path, err)
	}
...
func spoolNar(tb testing.TB, spoolDir, name string, narBytes []byte) string {
	tb.Helper()
...
func fakeNixStore(t *testing.T, body string) {
	...
	require.NoError(t, os.Symlink(executable, filepath.Join(dir, "test-helper")))
	bodyPath := filepath.Join(dir, "body.sh")
	require.NoError(t, os.WriteFile(bodyPath, []byte(body+"\ncat >/dev/null\n"), 0o600))

	launcher := `#!/bin/sh
exec "$(dirname "$0")/test-helper" -test.run='^TestServeProcessHelper$' -- "$@"
`
	require.NoError(t, os.WriteFile(filepath.Join(dir, "nix-store"), []byte(launcher), 0o700)) // #nosec G306 -- executable test launcher.
	t.Setenv("TSNIXCACHE_NIX_BODY", bodyPath)
...
func TestImportRootSyncFailures(t *testing.T) {
	for _, failAt := range []int{1, 2} {
		t.Run(strconv.Itoa(failAt), func(t *testing.T) {
			data := buildNarFor(t, storeFixture(t))
			ni := narInfoFor(synthStorePath("sync"), testNarURL, data)
			spool := t.TempDir()
			spoolNar(t, spool, "test.nar", data)
			roots := t.TempDir()
			root := gcRootFor(roots, ni.StorePath)
			invoked := filepath.Join(t.TempDir(), "invoked")
			t.Setenv("TSNIXCACHE_TEST_INVOKED", invoked)
			fakeNixStore(t, `cat >/dev/null; touch "$TSNIXCACHE_TEST_INVOKED"`)

			injected := unix.EIO
			calls := 0

			t.Cleanup(gcroot.SetSyncDirForTesting(func(path string) error {
				_, err := os.Lstat(root)
				if path == roots && err == nil {
					calls++
					if calls == failAt {
						return injected
					}
			...
			imp := &Importer{SpoolDir: spool, GCRootDir: roots}
			require.ErrorIs(t, imp.Import(t.Context(), ni), injected)

			if failAt == 1 {
				require.NoFileExists(t, invoked)
			} else {
				require.FileExists(t, invoked)
			}

			target, err := os.Readlink(root)
			require.NoError(t, err)
			require.Equal(t, ni.StorePath, target)

			lock, err := gcroot.TryAcquire(roots, filepath.Base(root))
			require.NoError(t, err)
...
func TestImportReportsRollbackFailure(t *testing.T) {
	data := buildNarFor(t, storeFixture(t))
	ni := narInfoFor(synthStorePath("rollback"), testNarURL, data)
	spool := t.TempDir()
	spoolNar(t, spool, "test.nar", data)
	roots := t.TempDir()
	root := gcRootFor(roots, ni.StorePath)
	t.Setenv("TSNIXCACHE_TEST_ROOT", root)
	fakeNixStore(t, `cat >/dev/null
rm "$TSNIXCACHE_TEST_ROOT"
mkdir "$TSNIXCACHE_TEST_ROOT"
touch "$TSNIXCACHE_TEST_ROOT/block-removal"
exit 7`)
	err := (&Importer{SpoolDir: spool, GCRootDir: roots}).Import(t.Context(), ni)
...
func TestImportStreamsVerifiedBytes(t *testing.T) {
	...
	if bytes.Contains(sent, poison) {
		t.Error("unverified bytes reached nix-store --serve")
	}
...
func TestImportRefusesASpoolSymlinkOutOfTheSpool(t *testing.T) {
	...
	_, err = os.Stat(ran)
	if err == nil {
		t.Error("nix-store --serve ran on content reached through a symlink out of the spool")
	}
...
// Nix processes sharing one fail with "database is busy" — a property of
...
func TestVerifyCancellationAndSizeLimit(t *testing.T) {
	data := bytes.Repeat([]byte("nar"), 1024)

	for _, codec := range []string{"none", "zstd", "xz", compressionBzip2} {
		t.Run(codec, func(t *testing.T) {
			if (codec == "xz" || codec == compressionBzip2) && !nixcompress.ExternalAvailable(codec) {
				t.Skip("codec unavailable")
			}

			var compressed bytes.Buffer

			enc, err := nixcompress.Encoder(t.Context(), &compressed, codec, false)
			if err != nil {
				t.Fatal(err)
			}
			...
			f, err := os.CreateTemp(t.TempDir(), "nar")
			if err != nil {
				t.Fatal(err)
			}
			defer f.Close()
			...
			ni := narInfoFor(testStorePath, testNarURL, data)
			ni.Compression = codec
			ctx, cancel := context.WithCancel(t.Context())
			cancel()

			_, err = (&Importer{}).verify(ctx, f, ni)

			if !errors.Is(err, context.Canceled) {
				t.Fatalf("cancelled verification: %v", err)
			}

			_, err = (&Importer{MaxNarSize: uint64(len(data)) - 1}).verify(t.Context(), f, ni)

			if !errors.Is(err, errNarTooLarge) {
			...
			_, err = (&Importer{MaxNarSize: uint64(len(data))}).verify(t.Context(), f, ni)
			if err != nil {
				t.Fatalf("at limit: %v", err)
			}

			ni.NarSize = math.MaxUint64

			_, err = (&Importer{MaxNarSize: math.MaxUint64}).verify(t.Context(), f, ni)
			if !errors.Is(err, errNarTooLarge) {
...
func TestOverlappingImportRootOwnership(t *testing.T) {
	data := buildNarFor(t, storeFixture(t))
	ni := narInfoFor(synthStorePath("root-ownership"), testNarURL, data)
	firstSpool, secondSpool, roots, signals := t.TempDir(), t.TempDir(), t.TempDir(), t.TempDir()
	spoolNar(t, firstSpool, "test.nar", data)
	spoolNar(t, secondSpool, "test.nar", data)

	started := filepath.Join(signals, "started")
	t.Setenv("TSNIXCACHE_IMPORT_STARTED", started)
	// Hold the first import at an explicit FIFO barrier, independently of scheduling.
	fifo := filepath.Join(signals, "release")
	require.NoError(t, unix.Mkfifo(fifo, 0o600))
	fakeNixStore(t, "if [ \"$2\" = first ]; then touch \"$TSNIXCACHE_IMPORT_STARTED\"; read line <\"$TSNIXCACHE_IMPORT_FIFO\"; exit 1; fi")
	t.Setenv("TSNIXCACHE_IMPORT_FIFO", fifo)

	first := &Importer{SpoolDir: firstSpool, GCRootDir: roots, NixStoreURI: "first"}
	second := &Importer{SpoolDir: secondSpool, GCRootDir: roots, NixStoreURI: "second"}

	ctx, cancel := context.WithTimeout(t.Context(), 5*time.Second)
	defer cancel()

	var group errgroup.Group
	group.Go(func() error { return first.Import(ctx, ni) })
	...
	lock, err := gcroot.TryAcquire(roots, first.storePathHash(ni.StorePath))
	require.ErrorIs(t, err, gcroot.ErrBusy)
	require.Nil(t, lock)

	waitCtx, waitCancel := context.WithTimeout(ctx, 25*time.Millisecond)
	defer waitCancel()

	require.ErrorIs(t, second.Import(waitCtx, ni), context.DeadlineExceeded)

	f, err := os.OpenFile(fifo, os.O_WRONLY|unix.O_NONBLOCK, 0) // #nosec G304 -- test FIFO under t.TempDir.
	require.NoError(t, err)
	...
	spoolNar(t, secondSpool, "test.nar", data)
	require.NoError(t, second.Import(ctx, ni))
	_, err = os.Lstat(gcRootFor(roots, ni.StorePath))
	require.NoError(t, err)
}
...
// Run a prepared operation in a fresh process so fixture creation cannot inflate RSS.
func TestImportResourceProbe(t *testing.T) {
	if os.Getenv("TSNIXCACHE_RESOURCE_PROBE") == "" {
		t.Skip("resource evidence runs explicitly with prepared disk fixtures")
	}

	if runtime.GOOS != "linux" {
		t.Skip("resource counters use Linux units and procfs")
	}
	...
	data, err := os.ReadFile("resource-config.json")
	require.NoError(t, err)
	...
	spool := filepath.Join(directory, "spool")
	uri := filepath.Join(directory, "store")

	metadata := filepath.Join(directory, "fixture.narinfo")

	if cfg.Operation == "prepare" {
		require.NoDirExists(t, uri, "resource preparation requires a fresh store")
		require.NoError(t, os.MkdirAll(spool, 0o700))
		raw := benchNar(t, cfg.PayloadBytes, cfg.High)
		encoded, ni := benchEncoded(t, raw, cfg.Compression)
		spoolNar(t, spool, "bench.nar", encoded)

		data = []byte(ni.Marshal())
		require.NoError(t, os.WriteFile(metadata, data, 0o600))
		out, initErr := exec.CommandContext(t.Context(), "nix-store", "--store", uri, "--init").CombinedOutput() // #nosec G204 -- fixed command and explicit resource fixture.
		require.NoError(t, initErr, "%s", out)

		return
	}

	data, err = os.ReadFile(metadata) // #nosec G304 -- explicitly selected local resource fixture.
	require.NoError(t, err)
	meta, err := narinfo.Parse(bytes.NewReader(data), narinfo.DefaultStoreDir)
	require.NoError(t, err)
	require.Equal(t, synthStorePath("bench"), meta.StorePath)
	ni := &narinfo.NarInfo{StorePath: synthStorePath("bench"), URL: "nar/bench.nar", Compression: meta.Compression, NarHash: meta.NarHash, NarSize: meta.NarSize, FileHash: meta.FileHash, FileSize: meta.FileSize}

	imp := &Importer{SpoolDir: spool, GCRootDir: "roots", NixStoreURI: uri, UseExternal: cfg.External}
	if cfg.External {
		require.True(t, nixcompress.ExternalAvailable(cfg.Compression))
	}

	var before, childBefore, after, childAfter unix.Rusage
	require.NoError(t, unix.Getrusage(unix.RUSAGE_SELF, &before))
	require.NoError(t, unix.Getrusage(unix.RUSAGE_CHILDREN, &childBefore))
	ioBefore := resourceIO(t)
	started := time.Now()

	switch cfg.Operation {
	case "verify":
		f, openErr := os.Open(filepath.Join(spool, "bench.nar")) // #nosec G304 -- explicitly selected local resource fixture.
		require.NoError(t, openErr)
		_, err = imp.verify(t.Context(), f, ni)
		require.NoError(t, f.Close())
	case "import":
		err = imp.Import(t.Context(), ni)
	default:
		t.Fatalf("unknown operation: %s", cfg.Operation)
	}
	...
	require.NoError(t, unix.Getrusage(unix.RUSAGE_CHILDREN, &childAfter))

	ioAfter := resourceIO(t)
	for key, value := range ioBefore {
		ioAfter[key] -= value
	}

	seconds := func(value unix.Timeval) float64 { return float64(value.Sec) + float64(value.Usec)/1e6 }
	result := map[string]any{
		"elapsed_seconds":           elapsed.Seconds(),
		"self_user_seconds":         seconds(after.Utime) - seconds(before.Utime),
		"self_system_seconds":       seconds(after.Stime) - seconds(before.Stime),
		"children_user_seconds":     seconds(childAfter.Utime) - seconds(childBefore.Utime),
		"children_system_seconds":   seconds(childAfter.Stime) - seconds(childBefore.Stime),
		"self_max_rss_kib":          after.Maxrss,
		"largest_child_max_rss_kib": childAfter.Maxrss,
		"worker_proc_io_delta":      ioAfter,
		"nar_bytes":                 ni.NarSize, "compressed_bytes": ni.FileSize,
	}
	data, err = json.Marshal(result)
	require.NoError(t, err)
	require.NoError(t, os.WriteFile(filepath.Join(directory, "resource.json"), data, 0o600))
}

func resourceIO(t *testing.T) map[string]int64 {
	t.Helper()
