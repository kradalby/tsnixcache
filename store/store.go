...
// maxConns caps both halves of the connection pool. Every SQLite connection
// carries its own libc thread state, page cache — roughly 5 MB against a real
// store DB — and three file descriptors, so an unbounded pool turns a burst of
// narinfo lookups into resident memory. It is also simply faster: measured
// against the real 97k-path DB, capping at 8 gave 41,615 lookups/s at 24 us
// versus 21,470/s at 47 us unbounded, and a larger cap (64) was no better than
// unbounded. Small pool, warm connections, prepared statements that survive.
const maxConns = 8
