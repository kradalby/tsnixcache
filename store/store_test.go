...
//
// It EXPLAINs store.QueryMain, the statement PathInfo actually prepares, so any
...
WHERE path LIKE ?`
	...
	got := plan("EXPLAIN QUERY PLAN"+store.QueryMain,
		storeDir+"/"+hashPart1+"-", storeDir+"/"+hashPart1+".")
	if !strings.Contains(got, "USING INDEX") {
		t.Errorf("plan for store.QueryMain = %q, want an index lookup", got)
	}
...
func TestPoolBoundsOpenConnections(t *testing.T) {
	...
	got := s.Stats().MaxOpenConnections
	if got < 4 || got > 8 {
...
func TestOpenWALReadOnlyDirExplains(t *testing.T) {
	...
	_, err = store.Open(t.Context(), dbPath, storeDir)
	if err == nil {
...
func TestMissingDB(t *testing.T) {
	_, err := store.Open(context.Background(), "/nonexistent/path/to/db.sqlite", storeDir)
	if err == nil {
