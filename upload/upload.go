...
var errResponseTooLarge = errors.New("upload response body exceeds limit")

var errIncompleteNar = errors.New("server answered before the NAR stream completed")
...
type Options struct {
	...
	OnPath       func(Stats)   // called serially once per path within a closure
...
func Closure(ctx context.Context, targetURL string, roots []string, opts Options) ([]Stats, Summary, error) {
	...
	sum := Summary{Paths: len(stats), Wall: time.Since(start)}
...
// Cover the server's 30-second slot wait and 30-minute import deadline.
// Expiration ends this push without repeating another full timeout.
...
// advance decrements the in-degree of every dependent of the completed paths
// and returns dependents that just became ready.
func (g *graph) advance(completed []string) []string {
	next := make([]string, 0)
...
// run advances each dependency as soon as it finishes, without delaying unrelated work.
func (u *uploader) run(ctx context.Context, metas []pathMeta) []Stats {
	g := buildGraph(metas)
	stats := make([]Stats, 0, len(g.byPath))
	recorded := make(map[string]bool, len(g.byPath))
	failed := make(map[string]bool)
	record := func(s Stats) {
		stats = append(stats, s)
		recorded[s.Path] = true

		failed[s.Path] = s.Err != nil
		if u.onPath != nil {
			u.onPath(s)
		}
	}
	jobs := min(max(u.jobs, 1), len(g.byPath))
	results := make(chan Stats, jobs)
	...
	for len(ready) > 0 || active > 0 {
		for len(ready) > 0 && active < jobs {
			...
			if err != nil {
				record(Stats{Path: path, NarSize: m.NarSize, Err: err})
				ready = append(ready, g.advance([]string{path})...)
		...
		record(s)
		ready = append(ready, g.advance([]string{s.Path})...)
	}
	...
	for p, m := range g.byPath {
		if !recorded[p] {
			record(Stats{Path: p, NarSize: m.NarSize, Err: errRefCycle})
		}
	}
...
func (u *uploader) putNar(ctx context.Context, storePath, narName string) (int64, string, float64, error) {
	...
	producerCtx, cancelProducer := context.WithCancel(reqCtx)
	pr, pw := io.Pipe()

	var producer errgroup.Group

	defer func() { cancelProducer(); _ = pr.Close(); _ = producer.Wait() }()
	...
	producer.Go(func() error {
		// The producer owns hashing until joined; HTTP responses can arrive before body EOF.
		enc, err := nixcompress.Encoder(producerCtx, io.MultiWriter(pw, h), narCompression, false)
		if err != nil {
		...
		err = errors.Join(nar.Write(&countWriter{w: enc, n: &sent}, storePath), enc.Close())
		_ = pw.CloseWithError(err)
	...
	var (
		monitor errgroup.Group
		peak    float64
	)

	monitor.Go(func() error {
		peak = u.monitor(storePath, m, &sent, cancel, done)
	...
	responseErr := u.readResponse(reqCtx, cancel, "put nar", resp)

	if !complete {
		producerErr = errors.Join(producerErr, errIncompleteNar)
	}

	err = errors.Join(responseErr, producerErr)
	if err != nil {
		return 0, "", 0, err
	}
...
func (u *uploader) putNarInfo(ctx context.Context, hashPart string, ni *narinfo.NarInfo) error {
	...
	req, err := http.NewRequestWithContext(bodyCtx, http.MethodPut,
	...
	err = u.readResponse(bodyCtx, bodyCancel, "put narinfo", resp)
...
// readResponse bounds diagnostics and the response phase independently of import time.
func (u *uploader) readResponse(
	ctx context.Context, cancel context.CancelCauseFunc, op string, resp *http.Response,
) error {
	timer := time.AfterFunc(orDefault(u.stallTimeout, 60*time.Second), func() { cancel(errStalled) })
	defer timer.Stop()
	...
	if resp.StatusCode == http.StatusOK {
		if readErr != nil {
			return fmt.Errorf("%s: response body: %w", op, readErr)
		}

		return nil
	}

	return statusError(op, resp, body, readErr)
}

// Preserve permanent status classification even when its diagnostic body fails.
func statusError(op string, resp *http.Response, body []byte, readErr error) error {
